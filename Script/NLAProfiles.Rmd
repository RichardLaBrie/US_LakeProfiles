...
#Remove empty lines
data0712 = data0712[!is.na(data0712$site_id),]
...
#Remove 2 lakes that were sampled twice in 2007 that were still in the database
data0712 = data0712[-c(59,67),]

#Remove 2012 lakes that were resampled at surface only
list12 = list()
data0712[order(data0712[,"site_id"], data0712[,"year"]), ]
for(i in 2:dim(data0712)[1]){
  if(data0712$site_id[i] != data0712$site_id[i-1]) next
  
  if( abs(data0712$sampled_depthmax_m[i]-data0712$sampled_depthmax_m[i-1]) >= 5) list12[[i]]=i
}

#Manually select the lakes to remove when sampling was obviously badly done
# View(data0712[c(unlist(list12),unlist(list12)-1),]) #Copy-pasted in Excel for easier read
#Lines to remove
L.rm = c(182,203,233,273,345,353,419,514,527,580,605,637,687,797,923,939,942,957,979, 991, 1040, 1051,1309,1352)

data0712 = data0712[-L.rm,]
...
#No hypolimnion = 0
HypoTop07 = ifelse(strata07[3,] != -1 & strata07[3,] != strata07[4,],  strata07[3,], 0) #m
HypoThick07 = ifelse(strata07[3,] != -1 & strata07[3,] != strata07[4,],  strata07[4,] - strata07[3,], 0) #m
BinHypo07 = ifelse(unlist(lapply(X = HypoThick07,FUN = function(X) X!=0)), 1, 0)
names(HypoTop07) <- names(HypoThick07) <- names(BinHypo07) <- colnames(strata07)

HypoTop12 = ifelse(strata12[3,] != -1 & strata12[3,] != strata12[4,],  strata12[3,], 0) #m
HypoThick12 = ifelse(strata12[3,] != -1 & strata12[3,] != strata12[4,],  strata12[4,] - strata12[3,], 0) #m
BinHypo12 = ifelse(unlist(lapply(X = HypoThick12,FUN = function(X) X!=0)), 1, 0)
names(HypoTop12) <- names(HypoThick12) <- names(BinHypo12) <- colnames(strata12)

Hypo.df07 <- data.frame(HypoTop = HypoTop07,
                        HypoThick = HypoThick07,
                        BinHypo = BinHypo07)
Hypo.df07$site_id = rownames(Hypo.df07)
Hypo.df12 <- data.frame(HypoTop = HypoTop12,
                        HypoThick = HypoThick12,
                        BinHypo = BinHypo12)
Hypo.df12$site_id = rownames(Hypo.df12)

#Combine with main dataframe by year
Hypo.df07$site_id = str_replace(string = Hypo.df07$site_id, pattern = "\\.", replacement = "-")
Hypo.df12$site_id = str_replace(string = Hypo.df12$site_id, pattern = "\\.", replacement = "-")

missdata07 = missdata0712[missdata0712$year=="2007",]
missdata12 = missdata0712[missdata0712$year=="2012",]

missdata07 = merge(missdata07, Hypo.df07, by = "site_id")
missdata12 = merge(missdata12, Hypo.df12, by = "site_id")

missdata0712 = rbind(missdata07,missdata12)
rm(missdata07)
rm(missdata12)
missdata0712$HypoProp = missdata0712$HypoThick / missdata0712$depthmax_m
